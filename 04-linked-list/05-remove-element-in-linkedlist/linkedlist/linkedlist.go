...
//输出索引的元素
func (linked *LinkedList) Remove(index int) (v interface{}, err error) {
	if index < 0 || index >= linked.size {
...
//输出链表头部元素
func (linked *LinkedList) RemoveFirst() (interface{},error) {
	return linked.Remove(0)
}

//删除链表尾部元素
func (linked *LinkedList) RemoveLast() (interface{},error) {
	return linked.Remove(linked.size -1 )
}

//清空链表
func (linked *LinkedList) Clear() {
	//linked.dummyHead = nil
	linked.dummyHead = newNode(nil)
	linked.size = 0
}
